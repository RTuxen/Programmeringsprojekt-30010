...
volatile uint8_t flag;      //Flag til LCD update
volatile uint8_t slide;     //Variabel til LCD update
volatile uint8_t gameflag;  //Flag til spil update
volatile uint8_t updateGame;//Variabel til spil update
...
void TIM2_IRQHandler(void){
    ...
    slide++;
    updateGame++;
    if (updateGame >= 4){//Spillet updateres med 25Hz
        gameflag = 1;
        updateGame = 0;
    }
